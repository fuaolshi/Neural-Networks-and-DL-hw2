_base_ = ['../_base_/schedules/schedule_1x copy1.py', '../_base_/default_runtime copy1.py']
# model settings
...
'''增加VOC数据集，在 metainfo 中，常见的键值对包括：
classes: 类别名称列表，每个元素是一个类别名称的字符串。
palette: 调色板，用于可视化时不同类别的颜色，通常是一个包含 RGB 值的列表。'''
...
val_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + 'annotations/val2017.json', # 更改文件位置
    metric='bbox',
    backend_args=backend_args)
test_evaluator = val_evaluator

train_cfg = dict(max_epochs=273, val_interval=7) # 更改max_epoch，interval，每7epoch验证一次
